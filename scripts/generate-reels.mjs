...
const STAGING_DIR = join(CARDS_DIR, '.staging');  // tiktok PNGs (temp, from generate-kanji-cards)
const REELS_DIR   = join(CARDS_DIR, 'reels');      // final MP4 output only
...
for (const kanji of targetKanji) {
  ...
  if (![card1, card2, card3].every(existsSync)) {
    console.warn(`⚠️  Cartes manquantes pour ${kanji} dans .staging/ — génère-les d'abord`);
    continue;
  }
...
try { require('fs').rmdirSync(STAGING_DIR); } catch {}
